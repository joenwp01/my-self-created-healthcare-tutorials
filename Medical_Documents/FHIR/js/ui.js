...
window.UI = {
  ...
  renderChoiceState, collapsible
};

/* ===== Collapsible lecture subsection helper =====
   Stations use this to wrap optional-depth content (typically the multi-paragraph
   <h4>-headed sections in lectures). Default state is closed: the header is
   visible with a chevron; the body is hidden until clicked. Headers should
   carry the lesson on their own; bodies provide elaboration for users who want it.

   Usage in a station's lecture():
     ${UI.collapsible("Why version pinning matters",
        "<p>Claiming us-core-patient without a version is ambiguous...</p>")}
*/
function collapsible(headerText, bodyHtml) {
  return `<details class="lecture-section">
    <summary>${headerText}</summary>
    <div class="lecture-section-body">${bodyHtml}</div>
  </details>`;
}
