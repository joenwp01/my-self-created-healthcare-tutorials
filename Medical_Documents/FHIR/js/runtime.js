...
const Runtime = {
  ...
  renderAll(opts) {
    ...
    // Capture scroll BEFORE reflowing — restored after if preserveScroll true.
    // Used by Hint, Reveal, and choice-click handlers so the user stays in
    // place mid-decision instead of being jumped to the top of the station.
    const priorScroll = opts.preserveScroll
    ...
    root.innerHTML = `
      ...
      ${s.beforeAfter ? `
        <details class="ba-details">
          <summary class="ba-summary">
            <span class="ba-chevron">▶</span>
            <span class="ba-summary-label">Before / After payloads</span>
            <span class="ba-summary-hint">click to toggle</span>
          </summary>
    ...
    // Call any post-render hook (e.g. to draw animations, restore feedback)
    if (s.postRender) s.postRender();

    this.renderStationIndicators();

    if (opts.preserveScroll) {
      // Restore prior viewport so Hint/Reveal/choice clicks don't scroll-jump.
      window.scrollTo({ top: priorScroll, behavior: "auto" });
      // Optionally bring a target into view after restoring (used by Reveal
      // to ensure the highlighted correct choice is centered on screen).
      if (opts.scrollIntoView) {
        const target = document.querySelector(opts.scrollIntoView);
  ...
  renderStationIndicators() {
    ...
    const completedCount = this.stations.filter(s => State.isStationComplete(this.journeyId, s.number)).length;
    // Animated fill percentage. Rail is fully muted at 0%, fully accent-colored
    // at 100%. Denominator is (total - 1) so the rail tops out when the last
    // station is reached.
    const fillPct = total > 1 ? Math.max(0, Math.min(100, Math.round(100 * completedCount / (total - 1)))) : 0;

    const dots = this.stations.map((s, i) => {
    ...
    sidebar.innerHTML = `
      <div class="station-rail-wrapper">
        <div class="station-rail-bg"></div>
        <div class="station-rail-fill" style="height: ${fillPct}%;"></div>
        <div class="station-rail-dots">${dots}</div>
      </div>
    `;
  },
  ...
  showHint() {
    ...
    const wasUsed = State.isHintUsed(this.journeyId, s.number);
    State.markHint(this.journeyId, s.number);

    // Inline render: write directly into #hint-area without regenerating the
    // station DOM. No scroll change. The button label updates on the next
    // natural re-render (choice click, advance, etc.) — for cleanliness we
    // also flip its label here.
    const box = document.getElementById("hint-area");
    if (box) box.innerHTML = UI.renderHint(s.task.hint);
    if (!wasUsed) {
      const hintBtn = document.querySelector('.action-bar button[onclick="Runtime.showHint()"]');
      if (hintBtn) hintBtn.innerHTML = "💡 Hint (shown)";
    }
  },
  ...
  reveal() {
    ...
    // Preserve scroll AND scroll the correct choice card into view so the
    // user lands on the green-highlighted answer instead of being jumped
    // back to the lecture top.
    const correctSelector = this.journeyId === "consume"
      ? `#s${s.number}-choices .correct, #sp${s.number}-choices .correct`
      : `#sp${s.number}-choices .correct`;
    this.renderAll({ preserveScroll: true, scrollIntoView: correctSelector });
  },
